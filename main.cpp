...
Ray refract(const vec3f &I, const Hit &hit, float eta_t, float eta_i = 1.f) {
    float cosi = -std::max(-1.f, std::min(1.f, dot(I,hit.n)));
    if (cosi < 0) {
        Hit hit1 = Hit(hit);
        hit1.n = -hit.n;
    ...
    vec3f refract_orig = dot(refract_dir, hit.n) < 0 ? hit.point - hit.n * 1e-3 : hit.point + hit.n * EPS;
...
vec3f pixel_color(const Ray& ray, const std::vector<Primitive*> &primitives,
    ...
    for (const auto &light : lights) {
        ...
        vec3f shadow_orig = dot(light_dir, N) < 0 ? point - N * EPS : point + N * EPS; // checking if the point lies in the shadow of the lights[i]
